...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024 * 64):
    ...
    if viewdirs is not None:
        # if view direction is not None, perform view direction embedding
...
def render(
    ...
    **kwargs,
    ...
    if c2w is not None:
        ...
        rays_o, rays_d = get_rays(H, W, focal, c2w)  # (H, W, 3), (H,W,3)
    else:
    ...
    if use_viewdirs:
        ...
        viewdirs = torch.reshape(viewdirs, [-1, 3]).float()  # HW, 3
    ...
    # Create ray batch
    rays_o = torch.reshape(rays_o, [-1, 3]).float()  # HXW, 3
    rays_d = torch.reshape(rays_d, [-1, 3]).float()  # HXW, 3

    near, far = (
        near * torch.ones_like(rays_d[..., :1]),
        far * torch.ones_like(rays_d[..., :1]),
    )
    rays = torch.cat([rays_o, rays_d, near, far], -1)  # HXW, 8=3+3+1+1
    if use_viewdirs:
        rays = torch.cat([rays, viewdirs], -1)  # HxW, 8+3=11
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    dists = (
        z_vals[..., 1:] - z_vals[..., :-1]
    )  # bin distance, seems like the ray marching process
    #
    dists = torch.cat(
    ...
    # Equation (5) in the ECCV'20 paper
    weights = (
        alpha
        * torch.cumprod(
    ...
    # simulating the process of ray marching process,
    # the wieghts are accumulated to the previous 1-alpha values.
    # torch.cumprod will return the cumulative product of elements by following the equation
    # y_1 = x_1 (1.0)
    # y_2 = x_1*x_2
    # y_3 = x_1*x_2*x_3
    # y_i = x_1*x_2*x_3*...x_i
...
def render_rays(
    ...
    )  # [N_rays=HxW, 3] normalized rays_d?
    bounds = torch.reshape(ray_batch[..., 6:8], [-1, 1, 2])  # [N_rays, 1, 2]
    near, far = bounds[..., 0], bounds[..., 1]  # [-1,1]

    t_vals = torch.linspace(
        0.0, 1.0, steps=N_samples
    )  # split N_sample bins in 0.0, 1.0 in the near and far boundary
    if not lindisp:
    ...
    # z_vals expand the values to N_rays
    ...
    pts = (
        ...
        # get pts for each 
    )  # [N_rays, N_samples, 3] xyz coordinates on the ray with N_samples
